...
import com.fitlib.app.data.AbandonedWorkoutSummary
import com.fitlib.app.data.AppContainer
import com.fitlib.app.data.CheckpointAction
import com.fitlib.app.data.CheckpointResult
import com.fitlib.app.data.CustomExerciseDraft
import com.fitlib.app.data.DailyCoachMessage
import com.fitlib.app.data.EquipmentConflictItem
import com.fitlib.app.data.AdherenceSessionSignal
import com.fitlib.app.data.ExerciseDetail
import com.fitlib.app.data.ExerciseHistoryDetail
import com.fitlib.app.data.ExerciseSummary
import com.fitlib.app.data.ExerciseVideoLinks
import com.fitlib.app.data.FORMULA_A_LOWER_HIGH_REPS_FOCUS_KEY
import com.fitlib.app.data.FORMULA_A_LOWER_STRENGTH_FOCUS_KEY
import com.fitlib.app.data.FORMULA_A_UPPER_PULL_HIGH_REPS_FOCUS_KEY
import com.fitlib.app.data.FORMULA_A_UPPER_PULL_STRENGTH_FOCUS_KEY
import com.fitlib.app.data.FORMULA_A_UPPER_PUSH_HIGH_REPS_FOCUS_KEY
import com.fitlib.app.data.FORMULA_A_UPPER_PUSH_STRENGTH_FOCUS_KEY
import com.fitlib.app.data.FORMULA_B_GLUTES_HAMSTRINGS_HIGH_REPS_FOCUS_KEY
import com.fitlib.app.data.FORMULA_B_GLUTES_HAMSTRINGS_STRENGTH_FOCUS_KEY
import com.fitlib.app.data.FORMULA_B_REAR_SIDE_DELTS_HIGH_REPS_FOCUS_KEY
import com.fitlib.app.data.FORMULA_B_REAR_SIDE_DELTS_STRENGTH_FOCUS_KEY
import com.fitlib.app.data.FORMULA_B_UPPER_CHEST_HIGH_REPS_FOCUS_KEY
import com.fitlib.app.data.FORMULA_B_UPPER_CHEST_STRENGTH_FOCUS_KEY
import com.fitlib.app.data.HistoricalExerciseSet
import com.fitlib.app.data.HistoryShareFormat
import com.fitlib.app.data.HistoryReuseMode
import com.fitlib.app.data.HistorySummary
import com.fitlib.app.data.HistoryDetail
import com.fitlib.app.data.LibraryFacets
import com.fitlib.app.data.LibraryFilters
import com.fitlib.app.data.LocationMode
import com.fitlib.app.data.normalizeExerciseNote
import com.fitlib.app.data.normalizeWeeklyFrequency
import com.fitlib.app.data.normalizeWorkoutDurationMinutes
import com.fitlib.app.data.OnboardingDraft
import com.fitlib.app.data.PersonalDataExportPayload
import com.fitlib.app.data.PendingWorkoutShare
...
import com.fitlib.app.data.ProgramOverview
import com.fitlib.app.data.ProgramSetupDraft
import com.fitlib.app.data.ProgramStatus
import com.fitlib.app.data.ReadinessContext
import com.fitlib.app.data.RecommendationBias
...
import com.fitlib.app.data.SkippedExerciseFeedbackPrompt
import com.fitlib.app.data.SfrDebriefExercise
import com.fitlib.app.data.SfrTag
import com.fitlib.app.data.StrengthScoreSummary
import com.fitlib.app.data.TemplateSummary
...
import com.fitlib.app.data.TrainingSplitProgram
...
import com.fitlib.app.data.WorkoutFeedbackSignalType
import com.fitlib.app.data.WorkoutMovementInsight
import com.fitlib.app.data.WorkoutMuscleInsight
import com.fitlib.app.data.WorkoutPlan
import com.fitlib.app.data.WorkoutExerciseSetDraft
import com.fitlib.app.data.buildTodayCompletionFeedbackAbFlags
import com.fitlib.app.data.buildAdherenceCurrencySnapshot
import com.fitlib.app.data.toPendingWorkoutShare
...
data class ProgramWeekProgressSummary(
    ...
    val checkpointStatus: com.fitlib.app.data.CheckpointStatus?,
)
...
internal fun sessionSetFromHistoryReuseDraft(
    ...
    return SessionSet(
        ...
        recommendedWeight = com.fitlib.app.data.formatRecommendedWeight(draft.recommendedWeight ?: draft.weight),
        reps = draft.reps?.toString()
            ?: recommendedReps?.toString()
            ?: targetReps.substringBefore('-').trim().ifBlank { targetReps.trim() },
        weight = com.fitlib.app.data.formatRecommendedWeight(draft.weight ?: draft.recommendedWeight),
...
internal fun weekTypeLabel(weekType: com.fitlib.app.data.WeekType?): String? = when (weekType) {
    com.fitlib.app.data.WeekType.ACCUMULATION -> "Accumulation"
    com.fitlib.app.data.WeekType.INTENSIFICATION -> "Intensification"
    com.fitlib.app.data.WeekType.DELOAD -> "Deload"
    com.fitlib.app.data.WeekType.TEST -> "Test"
    null -> null
}
...
internal fun buildProgramProgressSummary(
    ...
    weeks: List<com.fitlib.app.data.PlannedWeek>,
    sessions: List<PlannedSession>,
    ...
    val weekSummaries = (1..highestWeekNumber).map { weekNumber ->
        ...
        val checkpointStatus = checkpointsByWeek[weekNumber]
            ...
            ?.let { statuses ->
                when {
                    statuses.all { it == com.fitlib.app.data.CheckpointStatus.COMPLETED } -> com.fitlib.app.data.CheckpointStatus.COMPLETED
                    statuses.any { it == com.fitlib.app.data.CheckpointStatus.PENDING } -> com.fitlib.app.data.CheckpointStatus.PENDING
                    else -> com.fitlib.app.data.CheckpointStatus.SKIPPED
                }
            }
    ...
    return ProgramProgressSummary(
        ...
        completedCheckpoints = checkpoints.count { it.status == com.fitlib.app.data.CheckpointStatus.COMPLETED },
...
class FitLibViewModel(private val container: AppContainer) : ViewModel() {
    var uiState by mutableStateOf(AppUiState())
        private set

    private var customExerciseNameLookupJob: Job? = null
    private val exercisePrescriptionEngine = com.fitlib.app.data.ExercisePrescriptionEngine()
    ...
    fun addSessionSet(exerciseIndex: Int) {
        ...
        val updatedSets = exercise.sets + SessionSet(
            ...
            recommendationSource = template?.recommendationSource ?: com.fitlib.app.data.RecommendationSource.NONE,
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (activeProgram != null && plannedSession != null && workoutId != null) {
                ...
                container.programRepository.logEvent(
                    com.fitlib.app.data.ProgramEvent(
                        programId = activeProgram.id,
                        eventType = com.fitlib.app.data.ProgramEventType.SESSION_REALIZED,
    ...
    fun loadProgramState() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val pendingCheckpoints = allCheckpoints.filter { it.status == com.fitlib.app.data.CheckpointStatus.PENDING }
    ...
    fun startProgram(draft: ProgramSetupDraft) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val availableEquipment = com.fitlib.app.data.resolveAvailableEquipmentForGeneration(
    ...
    fun submitSfrFeedback(exerciseId: Long, tag: SfrTag) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            container.programRepository.logEvent(
                com.fitlib.app.data.ProgramEvent(
                    programId = program.id,
                    eventType = com.fitlib.app.data.ProgramEventType.SFR_UPDATED,
    ...
    fun acceptEvolutionSuggestion(slotId: Long, newExerciseId: Long) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            container.programRepository.logEvent(
                com.fitlib.app.data.ProgramEvent(
                    programId = program.id,
                    eventType = com.fitlib.app.data.ProgramEventType.EXERCISE_EVOLVED,
            ...
            CheckpointAction.PIVOT_EXERCISES -> {
                result.exerciseEvolutionSuggestions.firstOrNull()?.let { suggestion ->
                    container.programRepository.logEvent(
                        com.fitlib.app.data.ProgramEvent(
                            programId = program.id,
                            eventType = com.fitlib.app.data.ProgramEventType.PLAN_PIVOTED,
    ...
    private fun buildSessionExercise(
        exercise: WorkoutExercise,
        profile: UserProfile?,
        history: List<com.fitlib.app.data.HistoricalExerciseSet>,
    ): SessionExercise {
        ...
        return SessionExercise(
            ...
            sets = (1..exercise.sets).map { setNo ->
                val initialWeight = com.fitlib.app.data.formatRecommendedWeight(prescription.recommendedWeight)
                SessionSet(
                    setNumber = setNo,
...
class FitLibViewModelFactory(
    private val container: AppContainer,
) : ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(FitLibViewModel::class.java)) {
            @Suppress("UNCHECKED_CAST")
            return FitLibViewModel(container) as T
        }
